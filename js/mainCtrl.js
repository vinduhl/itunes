angular.module('itunes').controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = {
      ...
      columnDefs: [
        ...
        // Change the following to add target="_new" so that it will open a new window/tab instead of opening it in the same window.
        {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}" target="_new"><img src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.typeOptions = ["song", "feature-movie"];
      ...
      itunesService.getArtist( $scope.artist )
        .then( function( results ) {
          ...
          results = results.filter( function(val) {
            if($scope.selectedType && $scope.selectedType.trim() !== "") {
              return val.kind === $scope.selectedType;
            } else {
              return true;
            }
          });
